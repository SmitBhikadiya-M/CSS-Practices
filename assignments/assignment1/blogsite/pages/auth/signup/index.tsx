...
const Index = () => {
    ...
    async function onSignup(e: any){
        e.preventDefault();
        const registerRes = await fetch(`${process.env.NEXT_PUBLIC_AUTH_REST_ENDPOINT}/users/register`, {
            method: 'POST',
            headers: {
        ...
        router.push(`/auth/signin?userRegistered=true&callbackUrl=${router.query.callbackUrl}`);
        return;
    }
    ...
    if (status == "authenticated") {
        return <p className="w-full h-full text-center pt-3">You are already signed up do <button className='text-red-600 text-lg hover:text-red-700' onClick={()=>signOut()}>logout</button> first</p>
    }
            ...
            <div className="w-full max-w-sm">
                ...
                >
                    ...
                    <div className="flex items-center flex-col justify-between">
                        {error && (
                            <p className="pb-1 text-red-600">{error}</p>
                        )}
                        <button
